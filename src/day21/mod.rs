...
fn expand(pattern: &Array2<bool>, replacements: &HashMap<Array2<bool>, Array2<bool>>) -> Array2<bool> {
	...
	for (i1, i2) in (0..oldsize).step_by(oldstep as usize).zip((0..newsize).step_by(newstep as usize)) {
		for (j1, j2) in (0..oldsize).step_by(oldstep as usize).zip((0..newsize).step_by(newstep as usize)) {
			let source = pattern.slice(s![i1..(i1 + oldstep), j1..(j1 + oldstep)]);

			if let Some(target) = replacements.get(&source.to_owned()) {
				result.slice_mut(s![i2..(i2 + newstep), j2..(j2 + newstep)]).assign(target);
			}
		}
	}
...
crate fn solve() {
	...
	println!("[Part 2] Pixels on after 18 iterations: {}", pattern.iter().filter(|&&cell| cell).count());
}
