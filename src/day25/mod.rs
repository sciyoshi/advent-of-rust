...
pub fn solve() {
	...
	let (start, steps, states) = ws!(input.as_str(), do_parse!(
		...
		states: many0!(ws!(do_parse!(
			tag_s!("In state") >>
			state: terminated!(anychar, tag_s!(":")) >>
			transitions: many1!(ws!(do_parse!(
		...
		((start, steps, states.into_iter().collect::<HashMap<char, Vec<(usize, Direction, char)>>>()))
	)).to_result().unwrap();
